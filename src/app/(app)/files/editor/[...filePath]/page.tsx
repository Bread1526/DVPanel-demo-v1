...
export default function FileEditorPage() {
  ...
  const decodedFilePath = useMemo(() => {
    ...
    // Handle case where encodedFilePathFromParams might be a single string if not using [...filePath] but just [filePath]
    ...
    try {
      ...
      // setError("Invalid file path in URL. Failed to decode."); // Optionally set error state
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
            ...
            else errData = { error: `Failed to fetch snapshots. Status: ${response.status}. Server sent an empty response.`};
        } catch (e) {
             errData = { error: `Failed to fetch snapshots. Status: ${response.status}. Response: ${response.statusText || "Could not read error response."}` };
        }
  ...
  const fetchFileContent = useCallback(async () => {
    ...
    try {
      ...
      if (!isImage) {
          ...
          // For images, content is fetched directly by the <img> tag's src.
          // Set these to empty to avoid issues, or handle differently if image metadata is needed.
          setFileContent('');
      ...
      if(!isImage) { // Only fetch snapshots for non-image files
        ...
        // console.error("[FileEditorPage] Error in fetchFileContent:", e);
  ...
  }, [decodedFilePath, fileName, fetchSnapshots]); // Added fetchSnapshots
    ...
    try {
      ...
      try {
          if(responseText) {
              ...
              result = { snapshots: serverSnapshots, message: `Snapshot for ${fileName} created (empty server response).` }; 
          } else { 
          ...
          if (response.ok) {
              ...
              result = { snapshots: serverSnapshots, message: `Snapshot for ${fileName} created (response parse error).` }; 
          } else {
      ...
      if (!response.ok) {
        const errorMsg = result.error || result.details || result.message || `API Error ${response.status}: Failed to create snapshot.`;
      ...
      if(Array.isArray(result.snapshots)) {
        ...
        await fetchSnapshots(); // Re-fetch if the response format is unexpected
      }
    } catch (e: any) {
  ...
  const handleSaveChanges = useCallback(async () => {
    ...
    try {
      ...
      // setError(e.message || "An unexpected error occurred while saving."); // Error toast now handled by useEffect on error state
  ...
  const performSearch = useCallback((query: string) => {
    ...
    const cursor = new SearchCursor(view.state.doc, searchConfig.literal, 0, view.state.doc.length, searchConfig.caseSensitive ? undefined : (a,b) => a.toLowerCase() === b.toLowerCase());
  ...
  const handleToggleLockSnapshot = useCallback(async (snapshotId: string) => {
    ...
    setTimeout(() => toast({ 
        title: snapshot && !snapshot.isLocked ? "Snapshot Locked" : "Snapshot Unlocked", 
        description: "Lock status updated client-side. Server persistence not yet implemented."
    }), 0);
  ...
  const handleDeleteSnapshot = useCallback(async (snapshotIdToDelete: string) => {
    setServerSnapshots(prev => prev.filter(s => s.id !== snapshotIdToDelete));
     setTimeout(() => toast({ title: "Snapshot Deleted (Client)", description: "Server-side deletion not yet implemented."}), 0);
  }, [toast]);
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-96 max-w-[90vw]">
              ...
              <DropdownMenuLabel className="text-xs text-muted-foreground px-2 whitespace-normal">
                Snapshots are stored on the server. Locked snapshots are less likely to be auto-pruned. (Note: Server-side lock/delete persistence not yet implemented.)
              </DropdownMenuLabel>
            </DropdownMenuContent>
          </DropdownMenu>
