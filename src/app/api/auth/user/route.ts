...
import { loadPanelSettings } from '@/app/(app)/settings/actions'; // Corrected import path for function
import { type PanelSettingsData } from '@/app/(app)/settings/types'; // Corrected import path for type
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Also update lastActivity in the iron-session cookie data for consistency and to refresh its maxAge
  ...
  try {
    ...
    if (loadedUserSettings) {
      ...
      if (parsed.success) {
        ...
        console.warn(`[API /auth/user] User settings file ${userSettingsFilename} for ${effectiveUsername} is invalid. Using defaults. Errors:`, parsed.error.flatten().fieldErrors);
      }
    } else if (globalDebugModeForApi) {
  ...
  const authenticatedUser: AuthenticatedUser = {
      ...
      globalDebugMode: panelGlobalSettingsResult.data?.debugMode ?? false,
      isImpersonating: session.isImpersonating,
      originalUsername: session.isImpersonating ? session.originalUsername : undefined,
  };

  const userDebugMode = userSpecificSettings.debugMode; // This is the user's specific debug setting
  if (userDebugMode || globalDebugModeForApi) { // Log if user's own debug is on OR global panel debug is on
