  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn( // Base classes for positioning and initial state, specific sizing will be applied by EditorDialog
        ...
        className // Allows overriding or extending classes from the instance
      )}
      {...props}
    >
      {children}
      {!hideCloseButton && ( // This default X button will be hidden by EditorDialog passing hideCloseButton={true}
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 text-center sm:text-left", // Default ShadCN header styling
      className
    )}
    {...props}
  />
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2", // Default ShadCN footer styling
      className
    )}
    {...props}
  />
